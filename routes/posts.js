...
// New
router.get('/new', util.isLoggedin, async function(req, res){
  var post = req.flash('post')[0] || {};
  var errors = req.flash('errors')[0] || {};
  
  var styleTypes = await Style.find({}).sort('-createdAt').exec();
  var ingredientTypes = await Ingredient.find({}).sort('-createdAt').exec();
...
router.post('/', util.isLoggedin, upload.single('attachment'), async function(req, res){
  ...
  Post.create(req.body, function(err, post){
    if(err){
      ...
      return res.redirect('/admin/posts/new'+res.locals.getPostQueryString());
    }
    if(attachment){
      attachment.postId = post._id;
      attachment.save();
    }
    res.redirect('/admin/posts'+res.locals.getPostQueryString(false, { page:1, searchText:'' }));
  });
});
...
router.get('/:id', function(req, res){
  ...
  Promise.all([
    ...
    .then(([post, comments, style, ingredient]) => {
      post.views++;
      post.save();
...
// edit
router.get('/:id/edit', util.isLoggedin, checkPermission, async function(req, res){
  var post = req.flash('post')[0];
  var errors = req.flash('errors')[0] || {};
  ...
  if(!post){
    Post.findOne({_id:req.params.id})
      ...
      .exec(function(err, post){
        ...
        res.render('posts/edit', { post:post, errors:errors, styleTypes:styleTypes, ingredientTypes:ingredientTypes });
      });
  }
  else {
...
router.put('/:id', util.isLoggedin, checkPermission, upload.single('newAttachment'), async function(req, res){
  ...
  Post.findOneAndUpdate({_id:req.params.id}, req.body, {runValidators:true}, function(err, post){
    if(err){
      req.flash('post', req.body);
      req.flash('errors', util.parseError(err));
      return res.redirect('/admin/posts/'+req.params.id+'/edit'+res.locals.getPostQueryString());
    }
    res.redirect('/admin/posts/'+req.params.id+res.locals.getPostQueryString());
  });
});
...
router.delete('/:id', util.isLoggedin, checkPermission, function(req, res){
  Post.deleteOne({_id:req.params.id}, function(err){
    if(err) return res.json(err);
    res.redirect('/admin/posts'+res.locals.getPostQueryString());
  });
});
